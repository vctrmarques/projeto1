...
public class RestCandidatoController {
	...
	public @ResponseBody Resposta consultarProdutoPorId(@PathVariable String id) {
		Resposta resposta = new Resposta();
		try {
	...
	public @ResponseBody Resposta salvarProduto(@RequestBody Candidato candidato) {
		Resposta resposta = new Resposta();
		try {
			candidato.setId(null);
			candidatoService.salvarCandidato(candidato);
			resposta.setResposta(candidato); 
			logger.info(String.format("Salvando novo produto %s", candidato.getNome()));
		}catch(CandidatoException e) {
			resposta.setCodigo(Constantes.Status.CODIGO_ERRO);
			resposta.setMensagem(e.getMensagem());
		}
	...
	public @ResponseBody Resposta atualizarProduto(@RequestBody Candidato candidatoAtualizado, @PathVariable String id) {
		Resposta resposta = new Resposta();
		try {
	...
	public @ResponseBody Resposta removerProduto(@PathVariable String id) {
		Resposta resposta = new Resposta();
		try {
